...
public class taskInProgress extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        CustomAdapterTasksInProgress customAdapterTasksInProgress = new CustomAdapterTasksInProgress(getApplication(),mytasksId,mytasksSubject,mytasksDate
                ,mytasksTime);
        lv_taskInProgess.setAdapter(customAdapterTasksInProgress);

        lv_taskInProgess.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {

                Intent intent = new Intent(taskInProgress.this, taskDetails.class);
